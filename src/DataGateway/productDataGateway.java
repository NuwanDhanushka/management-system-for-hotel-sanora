...
public class productDataGateway {
    ...
    public void view(products products) {
        ...
        try {
            pst = connection.prepareStatement("SELECT SQL_NO_CACHE * FROM products");
            rs = pst.executeQuery();
            while (rs.next()) {
    ...
    public void viewFistTen(products products){
        ...
        products.ProductList.clear();
        try {
            pst = connection.prepareStatement("select * from products limit 0,15");
            rs = pst.executeQuery();
            while (rs.next()) {
                products.productId = rs.getString(1);
                products.productName = rs.getString(2);
    ...
    public void update(products products) {
       ...
       public void searchView(products products) {
        ...
        products.ProductList.clear();
        try {
            pst = connection.prepareStatement("select * from products where productName like ?");
            pst.setString(1, "%" + products.productName + "%");
            rs = pst.executeQuery();
            while (rs.next()) {
                products.productId = rs.getString(1);
